...
test("durable dispatcher skips events already covered by the stored cursor", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "council-talk-dispatcher-cursor-"));
  const eventStore = createAppendOnlyEventStore({
    filePath: path.join(tempDir, "events.jsonl"),
  });
  const handled = [];
  ...
  const cursorFilePath = path.join(tempDir, "cursor.json");
  await writeFile(
    cursorFilePath,
    JSON.stringify({
      lastSequence: 2,
